...
const (
	...
	clientRedisHost     = "redis-host"
	clientRedisPassword = "redis-password"
	clientRedisDb       = "redis-db"

	clientSqlAudit = "sql-audit"
...
func IsEnableSqlAudit() bool {
	if cltCfg != nil {
		return cltCfg.sqlAudit == 1
	}
	return false
}
...
func (this *clientConfig) loadclientConfig(c *conf.Conf) error {
	...
	this.redisHost = cMap[clientRedisHost]
	this.redisPassword = cMap[clientRedisPassword]
	redisDb := cMap[clientRedisDb]
	temp, err = strconv.Atoi(redisDb)
	if err != nil {
		appzaplog.Error("ParseInt redisDb failed", zap.Error(err), zap.String("redisDb", redisDb))
	}
	this.redisDb = temp

	sqlAudit := cMap[clientSqlAudit]
	temp, err = strconv.Atoi(sqlAudit)
	if err != nil {
		appzaplog.Error("ParseInt sqlAudit failed", zap.Error(err), zap.String("sqlAudit", sqlAudit))
	} else {
		this.sqlAudit = temp
	}
